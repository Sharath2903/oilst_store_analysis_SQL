----Schema----
DROP TABLE IF EXISTS oilst_customers;
CREATE TABLE IF EXISTS oilst_customers(
...
DROP TABLE IF EXISTS oilst_geolocation;
CREATE TABLE oilst_geolocation(
...
DROP TABLE IF EXISTS oilst_order_items;
CREATE TABLE oilst_order_items(
    order_id VARCHAR(50),
    order_item_id INT,
...
DROP TABLE IF EXISTS oilst_order_payments;
CREATE TABLE oilst_order_payments(
    order_id VARCHAR(50),
...
DROP TABLE IF EXISTS oilst_orders;
CREATE TABLE oilst_orders(
    order_id VARCHAR(50),
    customer_id VARCHAR(50),
...
DROP TABLE IF EXISTS oilst_order_ratings;
CREATE TABLE oilst_order_ratings(
...
DROP TABLE IF EXISTS oilst_sellers;
CREATE TABLE oilst_sellers(
    seller_id VARCHAR(50),
...
DROP TABLE IF EXISTS oilst_products;
CREATE TABLE oilst_products(
    product_id VARCHAR(50),
...
WITH total_sales AS (
    ...
    FROM 
        oilst_order_payments P
    INNER JOIN
        oilst_orders O ON P.order_id = O.order_id
    WHERE 
...
WITH yearly_revenue AS (
    ...
    INNER JOIN 
        oilst_orders O ON P.order_id = O.order_id
    INNER JOIN
        oilst_customers C ON O.customer_id = C.customer_id
    WHERE 
...
WITH delivery_analysis AS (
    ...
    FROM 
        oilst_orders O
    INNER JOIN
        oilst_order_ratings R ON O.order_id = R.order_id
    WHERE 
...
WITH negative_reviews AS (
    ...
    FROM  
        oilst_products P
    INNER JOIN
        oilst_order_items OI ON P.product_id = OI.product_id
    INNER JOIN
        oilst_order_ratings R ON OI.order_id = R.order_id
    GROUP BY
        P.product_category_name_eng
    HAVING 
...
INNER JOIN
    oilst_order_ratings R ON OI.order_id = R.order_id
INNER JOIN
    oilst_order_payments P ON OI.order_id = P.order_id
GROUP BY
    OI.seller_id
HAVING
...
WITH RFM_Calculation AS (
    ...
    FROM oilst_order_payments P
    JOIN oilst_orders O ON P.order_id = O.order_id
    JOIN oilst_customers C ON O.customer_id = C.customer_id
